      ...
      <div className="px-4 md:px-6 -mt-16 md:-mt-20 relative space-y-4">
        <div className="rounded-[32px] border border-[var(--app-border)] bg-[var(--app-surface)] shadow-[0_20px_60px_rgba(0,0,0,0.08)] p-4 md:p-6 space-y-5">
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
            ...
            <label className="avatar-3d relative w-28 h-28 md:w-36 md:h-36 rounded-full bg-[var(--app-foreground)] text-[var(--app-background)] flex items-center justify-center font-bold text-3xl md:text-5xl border-4 border-[var(--app-background)] shadow-xl overflow-hidden cursor-pointer group flex-shrink-0">
              {avatarFile ? (
            ...
            <div className="flex flex-col sm:flex-row gap-2 sm:items-center">
              <button
                ...
                className="px-4 py-2 bg-[var(--app-surface-raised)] border border-[var(--app-border)] rounded-full font-bold text-xs hover:bg-[var(--app-surface-soft)] hover:-translate-y-0.5 hover:shadow-md transition-all duration-200 shadow-sm flex items-center justify-center"
              >
              ...
              {!isStandalone && (
                <Link
                  href="/download"
                  className="px-4 py-2 bg-[var(--app-surface-raised)] border border-[var(--app-border)] rounded-full font-bold text-xs hover:bg-[var(--app-surface-soft)] hover:-translate-y-0.5 hover:shadow-md transition-all duration-200 shadow-sm flex items-center justify-center gap-1.5"
                >
              ...
              <button
                onClick={saveProfile}
                disabled={loading}
                className="px-5 py-2.5 bg-[var(--app-foreground)] text-[var(--app-background)] rounded-full font-black text-xs hover:opacity-90 hover:-translate-y-0.5 hover:shadow-xl transition-all duration-200 shadow-sm flex items-center justify-center disabled:opacity-50"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-w-3xl">
            <input value={displayName} onChange={(e) => setDisplayName(e.target.value)} className="rounded-2xl border border-[var(--app-border)] bg-[var(--app-surface-raised)] px-4 py-3 text-sm outline-none focus:border-[var(--app-accent)] transition" placeholder="Nom affiché" />
            <input value={city} onChange={(e) => setCity(e.target.value)} className="rounded-2xl border border-[var(--app-border)] bg-[var(--app-surface-raised)] px-4 py-3 text-sm outline-none focus:border-[var(--app-accent)] transition" placeholder="Ville" />
          </div>
          <textarea value={bio} onChange={(e) => setBio(e.target.value)} className="w-full max-w-3xl rounded-2xl border border-[var(--app-border)] bg-[var(--app-surface-raised)] px-4 py-3 text-sm outline-none min-h-28 focus:border-[var(--app-accent)] transition" placeholder="Bio" />
          <input value={headline} onChange={(e) => setHeadline(e.target.value)} className="w-full max-w-3xl rounded-2xl border border-[var(--app-border)] bg-[var(--app-surface-raised)] px-4 py-3 text-sm outline-none focus:border-[var(--app-accent)] transition" placeholder="Titre du profil" />

          {/* Bouton Changer la photo de profil */}
          <div className="pt-1">
            <label className="inline-flex items-center gap-2.5 px-4 py-2.5 rounded-2xl border border-[var(--app-border)] bg-[var(--app-surface-raised)] hover:bg-[var(--app-surface-soft)] hover:-translate-y-0.5 hover:shadow-md text-xs font-bold cursor-pointer transition-all duration-200 shadow-sm">
          ...
          {loading && <div className="text-sm text-neutral-500 animate-pulse">Chargement des données...</div>}
            ...
            <div className="space-y-3">
              ...
              {followers.length === 0 ? (
                ...
                <div className="space-y-2">
                  {followers.map((f) => (
                    <div key={f.id} onClick={() => router.push(`/profile/${f.id}`)} className="card-3d flex items-center gap-3 p-3 rounded-2xl border border-[var(--app-border)] hover:border-[var(--app-foreground)] cursor-pointer bg-[var(--app-surface-raised)] hover:bg-[var(--app-surface-soft)] transition-all duration-200 shadow-sm">
